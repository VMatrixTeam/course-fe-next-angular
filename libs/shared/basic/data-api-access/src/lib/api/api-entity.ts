...
import { AsyncRequestState, DataApiAccessService } from '@course-fe-next/shared/basic/data-api-access';
...
export class RouteBasedApiEntity<T extends ApiResponseBody> extends ApiEntity<T> {
  private readonly latestEndpoints$: Observable<ConstantEndpointsProvider> = this.router.events.pipe(
    filter((event) => event instanceof NavigationEnd),
    map(() => this.configuration.endpointsProvider.pathRegex.exec(this.router.url)),
    filter((result) => !!result),
    distinctUntilChanged(),
    map((result) => this.configuration.endpointsProvider.provideEndpoints((result as RegExpExecArray).slice(1))),
    shareReplay(1)
  );

  private readonly doGetOnRouteUpdateSubscription = this.latestEndpoints$.subscribe(() => this.doGet());
